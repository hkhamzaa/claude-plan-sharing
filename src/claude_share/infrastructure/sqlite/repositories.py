...
class SqliteDeviceRepository(DeviceRepository):
    ...
    def add(self, device: Device) -> None:
        self._conn.execute(
            "INSERT INTO devices (id, user_id, device_name, created_at, token_hash) VALUES (?, ?, ?, ?, ?)",
            (device.id, device.user_id, device.device_name, device.created_at.isoformat(), device.token_hash),
        )

    def get(self, device_id: str) -> Device | None:
        row = self._conn.execute(
            "SELECT id, user_id, device_name, created_at, token_hash FROM devices WHERE id = ?",
            (device_id,),
        ).fetchone()
        if row is None:
            return None
    ...
    def list_by_user(self, user_id: str) -> list[Device]:
        rows = self._conn.execute(
            "SELECT id, user_id, device_name, created_at, token_hash FROM devices WHERE user_id = ? ORDER BY created_at",
            (user_id,),
        ).fetchall()
    ...
    def find_by_token_hash(self, token_hash: str) -> Device | None:
        row = self._conn.execute(
            "SELECT id, user_id, device_name, created_at, token_hash FROM devices WHERE token_hash = ?",
            (token_hash,),
        ).fetchone()
        if row is None:
            return None
